...
PreTeXt book of *skeletal* (fill-in) lecture notes for SCU's MATH 14: the
definitions, theorem statements and problem statements of the completed
notes, with every derivation, proof and worked solution replaced by a blank
workspace box and a one-line instruction, so that students complete the
notes by hand in class (printed, or annotated on a tablet over the PDF that
the navbar's **PDF** button links to). The assignments and review problems
chapter is unchanged from the completed notes.

- `source/` — the book; `source/main.ptx` is the root.
- `scripts/build-site.sh` — builds the web book and the PDF, and puts the
  PDF next to the HTML pages; this is what CI runs.
- `CLAUDE.md` and `.claude/skills/skeletonize/` — how the sections were
  hollowed out, and the scripts to do it again.
